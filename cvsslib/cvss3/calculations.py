...
def calculate_base_score(run_calculation, scope: Scope, privilege: PrivilegeRequired):
    ...
    if impact_sub_score <= 0:
        ...
        if scope == Scope.CHANGED.value:
            return roundup(min(D("1.08") * combined_score, 10)), exploitability_sub_score
        else:
            return roundup(min(combined_score, 10)), exploitability_sub_score
...
def calculate(run_calculation, get):
    ...
    temporal_score = run_calculation(calculate_temporal_score, base_score[0])
    ...
    return float(base_score[0]), float(temporal_score), float(environment_score), float(base_score[1])
