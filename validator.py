...
from learner import LinearModel
...
class PerformanceValidator:
    ...
    def get_perf(data, schema, n_splits=5, selection_method=SelectionMethod.ShuffleSplit):
        ...
        X = [Utils.get_element_feature(schema, item) for item in data]
        Y = [Utils.get_target_function_value(data, item) for item in data]
        X = np.array(X)
        Y = np.array(Y)
        ...
        for train_index, test_index in selector.split(X):
            ...
            model = LinearModel(X_train, schema)
